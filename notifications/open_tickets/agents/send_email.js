...
const send_email = (user, ticket, time, type, cc) => {

    let to = user.email
    let subject = 'OPEN TICKET REMINDER - T-ID: ' + ticket.id
    let header1 = 'OPEN TICKET REMINDER - T-ID: '
    let header2 = ticket.id

    const link = 'https://test.precision.ke/agent/dashboard/my-assigned-tickets/details/'+ticket.id



    let message = `<p>Dear agent, please login to precision desk to solve the following ticket. ${type === 'hrs' ? time + ` hrs` : time + ` minutes`} have passed.</p>`+
